...
for (const file of files) {
  ...
  const creates = [...sql.matchAll(/create\s+(table|index|unique index)\s+([^\s(]+)/g)];
  ...
  const policies = [...sql.matchAll(/create\s+policy\s+"([^"]+)"/g)].map((m) => m[1]);
...
for (const file of files) {
  const sql = readSql(file);
  const tables = [
    ...sql.matchAll(/create\s+table\s+if\s+not\s+exists\s+public\.([a-z0-9_]+)/g),
  ].map((m) => m[1]);
  ...
  for (const table of tables) {
    ...
    check(
      ...
      new RegExp(`alter\\s+table\\s+public\\.${table}\\s+enable\\s+row\\s+level\\s+security`).test(
        sql,
      ),
    );
    ...
    check(
      ...
      new RegExp(`grant\\s+select\\s+on\\s+table\\s+public\\.${table}\\s+to\\s+authenticated`).test(
        sql,
      ),
    );
    check(
...
for (const file of files) {
  const sql = readSql(file);
  const fns = [...sql.matchAll(/create\s+(?:or\s+replace\s+)?function\s+public\.([a-z0-9_]+)/g)];
  check(
...
for (const file of files) {
  const sql = readSql(file);
  const updates = [...sql.matchAll(/update\s+public\.[a-z0-9_]+[\s\S]*?(?=;)/g)];
