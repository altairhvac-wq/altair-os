 ...
 * .env.local and writes failure columns onto a payment_attempts row. That is
 * fine when a developer runs this script deliberately against a scratch
 * project, and unacceptable inside the aggregate `verify:all` gate, which is
 * documented as offline and side-effect free and is expected to be safe to
 * run in any checkout -- including one whose .env.local points at production.
 *
