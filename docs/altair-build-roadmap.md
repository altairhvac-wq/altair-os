...
**Surface system:** Dashboard, Customers, Customer Profile, Leads, Jobs, Estimates, Invoices, and Payments use MC v2 light paper (`shared/design-system/components/mc-surface.ts`). Reports uses dark elevated chrome (`report-surface.ts`) plus categorical icon-chip tints (`report-icon-tints.ts`). See `.cursor/rules/altair-design-system.mdc`.
