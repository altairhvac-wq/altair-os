...
| Reviews collection system | Not built | No end-to-end path to request, collect, store, or aggregate customer reviews for technicians or the company. Team → Technicians shows an honest “No reviews yet” empty state only — do not invent stars, averages, or review counts. Related: no technician star/score model on team, jobs, or dispatch. |
| PTO / leave tracking | Not built | No PTO, time-off, or leave-balance model. Do not invent availability-from-PTO, leave calendars, or remaining-days metrics on Team → Technicians, team profile, or dispatch. |
| Referral attribution | Not built | `member_share_code` on `company_memberships` (migration `125`) is display/copy only on Team → Technicians and team profile. Do not claim lead/customer attribution, conversion counts, or “referred by” linkage from codes until tracking ships. Distinct from Network/Community B2B `network_referrals`. |
