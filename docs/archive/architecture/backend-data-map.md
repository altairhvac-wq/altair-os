> **⚠️ OUTDATED — Archived planning document.** This file describes pre-Supabase mock prototypes and lives under `docs/archive/architecture/`. All core modules are wired to Supabase in production. For confirmed inventory see `docs/product/ALTair_BRAIN.md`. For current state see `docs/product/ALTair_MASTER_STATUS.md`.
