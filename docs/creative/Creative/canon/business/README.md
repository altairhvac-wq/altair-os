# Business

Office, dispatch, and operations imagery.

## Naming

`ALT-BUS-NNN` (example: `ALT-BUS-001`)

IDs are permanent. Never recycle.

## Approved asset set

Every approved image receives:

1. **Image** — the visual file
2. **Matching Markdown** — caption, usage, and metadata
3. **Prompt History** — generation and revision record
4. **Approval Notes** — review decision and date

Track all assets in `docs/creative/Creative/CANON_IMAGE_LIBRARY.md`.
