...
import {
  LEAD_WORK_QUEUE_LABELS,
  LEAD_WORK_QUEUE_ORDER,
  type LeadWorkQueue,
} from "./lead-work-queues";

type LeadQueueTabsProps = {
  activeQueue: LeadWorkQueue;
  onQueueChange: (queue: LeadWorkQueue) => void;
  counts: Record<LeadWorkQueue, number>;
  northStar?: boolean;
};
...
export function LeadQueueTabs({
  activeQueue,
  onQueueChange,
  counts,
  northStar = false,
}: LeadQueueTabsProps) {
  const tabs = LEAD_WORK_QUEUE_ORDER.map((queue) => ({
    id: queue,
    label: LEAD_WORK_QUEUE_LABELS[queue],
    count: counts[queue],
  }));
