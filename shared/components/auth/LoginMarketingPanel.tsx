...
function HeroStatusCard({
  ...
  return (
    <div className={`absolute rounded-xl border border-[#c2a05a]/30 bg-[#0e100d]/88 px-3 py-2.5 shadow-[0_16px_36px_rgba(0,0,0,0.45)] backdrop-blur-xl ${className}`}>
...
function FieldServiceHero() {
  return (
    <figure className="auth-panel-enter relative overflow-hidden rounded-[1.4rem] border border-white/10 bg-[#131712] shadow-[0_28px_80px_-34px_rgba(0,0,0,0.9),0_0_0_1px_rgba(194,160,90,0.08)_inset]">
      <div className="relative aspect-[16/11] min-h-[220px] sm:min-h-[290px]">
        ...
        />
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#0a0b09]/75 via-transparent to-[#0a0b09]/10" />
...
function ControlLoop() {
  return (
    <section aria-labelledby="altair-control-loop-title" className="login-command-card relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#121611]/92 px-4 py-4 shadow-[0_24px_60px_-34px_rgba(0,0,0,0.9),0_0_0_1px_rgba(194,160,90,0.06)_inset] sm:px-5">
      <div className="login-scan-line pointer-events-none absolute inset-x-0 top-0 h-px" />
      <h2 id="altair-control-loop-title" className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#c2a05a]">
        Altair Control Loop
      </h2>
      <p className="mt-1 text-[10px] text-slate-400 sm:hidden">Swipe to follow the full workflow →</p>
      <div
        className="mt-3 overflow-x-auto pb-1 outline-none [scrollbar-width:none] focus-visible:ring-2 focus-visible:ring-[#c2a05a]/40 [&::-webkit-scrollbar]:hidden"
        tabIndex={0}
        role="region"
          ...
          <ol className="login-control-loop-track relative grid grid-cols-8 gap-1 pt-1">
            {WORKFLOW_STAGES.map((stage) => (
              <li key={stage.label} className="relative z-10 text-center">
                <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl border border-[#c2a05a]/28 bg-[#10120e] text-[#d9c188] shadow-[0_6px_18px_rgba(0,0,0,0.35),0_0_0_3px_rgba(194,160,90,0.035)]">
  ...
  return (
    <section aria-label="Altair benefits" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {BENEFITS.map((benefit) => (
        <article key={benefit.title} className="group flex min-h-44 flex-col rounded-2xl border border-white/[0.09] bg-gradient-to-br from-white/[0.055] to-white/[0.025] p-4 transition-all duration-300 hover:-translate-y-1 hover:border-[#c2a05a]/25 hover:bg-white/[0.06]">
          <div className="flex items-start gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#c2a05a]/20 bg-[#10120e] text-[#dabb55]">
          ...
          <p className="mt-auto pt-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#c2a05a]">{benefit.outcome}</p>
        </article>
      ))}
    </section>
  );
  ...
  return (
    <section aria-label="Why teams choose Altair" className="grid gap-px overflow-hidden rounded-2xl border border-white/[0.09] bg-white/[0.09] sm:grid-cols-2 xl:grid-cols-4">
      {TRUST_ITEMS.map((item) => (
        <div key={item.title} className="flex items-center gap-3 bg-[#10120e] px-4 py-3.5">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#c2a05a]/22 text-[#dabb55]">
...
export function LoginMarketingPanel() {
  return (
    <section className="relative min-h-full overflow-hidden bg-[#0a0b09] text-white">
      ...
      <div className="relative mx-auto max-w-[1160px] px-5 py-7 sm:px-8 sm:py-9 lg:px-8 xl:px-10">
        <header className="flex items-center justify-between gap-4">
          <AltairLogo variant="white" size="lg" showWordmark className="drop-shadow-[0_3px_18px_rgba(194,160,90,0.2)]" />
          <span className="inline-flex items-center gap-2 whitespace-nowrap rounded-full border border-white/10 bg-white/[0.045] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.13em] text-slate-300 backdrop-blur-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-[#c2a05a] shadow-[0_0_10px_rgba(194,160,90,0.7)]" />
        ...
        <div className="mt-8 grid items-center gap-7 xl:grid-cols-[0.88fr_1.12fr]">
          <div className="auth-hero-enter">
            <p className="text-[10px] font-semibold uppercase tracking-[0.19em] text-[#c2a05a]">Operating system for field service</p>
            <h2 className="mt-4 text-[clamp(2.4rem,4.5vw,4.5rem)] font-semibold leading-[1.02] tracking-[-0.05em] text-[#fafaf8]">{LOGIN_HERO_TITLE}</h2>
