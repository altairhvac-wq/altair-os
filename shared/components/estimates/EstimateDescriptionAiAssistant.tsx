    ...
    startTransition(async () => {
      ...
      if (result.error || !result.draftText?.trim()) {
        setError(
          formatActionError(
            result.error,
            "Could not rewrite the notes. Try again.",
          ),
        );
        return;
      }
    ...
    <div className="mt-2 space-y-1.5">
      ...
      {isPending ? (
        <p
          ...
          role="status"
        >
          Altair is drafting a customer-ready description…
        </p>
      ) : (
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          ...
          {previousNotes !== null ? (
            <button
              ...
              onClick={handleUndo}
              disabled={controlsDisabled}
              className="inline-flex min-h-9 items-center gap-1 rounded-md px-2 py-1.5 text-[11px] font-semibold text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-60 sm:min-h-8 sm:px-1.5 sm:py-1"
            >
