...
function NorthStarPanelHeader({
  ...
  return (
    <div className="expense-north-star-detail-panel-header flex shrink-0 items-center justify-end border-b border-[rgba(194,160,90,0.14)] bg-gradient-to-b from-[#2B3328] to-[#1C211A] px-3 py-2 sm:px-4">
      <button
        type="button"
        onClick={onClose}
      ...
      >
        ...
        <aside className="expense-north-star-detail-panel relative ml-auto flex h-full w-[min(720px,58vw)] min-w-[min(100%,520px)] flex-col overflow-hidden border-l border-[rgba(188,181,165,0.18)] bg-[#1C211A] shadow-[0_22px_60px_rgba(6,7,5,0.42)]">
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            {footer ? (
              <div className="overlay-form-actions shrink-0 border-t border-[rgba(194,160,90,0.14)] bg-[#1C211A] px-3 py-2.5 sm:px-4">
                {footer}
              </div>
            ) : null}
          </div>
  ...
  return (
    <>
      ...
      <aside className="expense-north-star-detail-panel-mobile flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:hidden">
        <NorthStarPanelHeader onClose={onClose} closeDisabled={closeDisabled} />
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[#1C211A]">
          <div className="expense-north-star-detail-panel-body flex min-h-0 flex-1 flex-col overflow-y-auto px-3 py-2.5">
            {children}
          </div>
          {footer ? (
            <div className="overlay-form-actions shrink-0 border-t border-[rgba(194,160,90,0.14)] px-3 py-2.5">
              {footer}
            </div>
          ) : null}
        </div>
