...
import type { CustomersGlanceStat } from "@/shared/lib/customers/customers-glance-stats";
import type { CustomerWorkQueue } from "./customer-work-queues";

type CustomersStatStripProps = {
  stats: CustomersGlanceStat[];
  activeQueue?: CustomerWorkQueue;
  onFilterQueue?: (queue: CustomerWorkQueue) => void;
};

/**
 * Compact inline glance stats for the Customers page header card.
 * Label + number only — no SectionHeader, card chrome, or tinted band.
 * Stats with `filterQueue` are pills that activate the matching work-queue filter.
 */
export function CustomersStatStrip({
  stats,
  activeQueue,
  onFilterQueue,
            ...
            <div key={stat.id} className="min-w-0 shrink-0">
              ...
              <dd className="m-0">
                <button
                  ...
                  aria-label={`${stat.label}: ${stat.value}. Filter customers.`}
