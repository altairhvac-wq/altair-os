...
function Skeleton({ className }: { className?: string }) {
  return (
    <div
      className={`north-star-skeleton rounded-lg border border-[rgba(119,89,27,0.12)] ${className ?? ""}`}
    />
  );
}
            ...
            <div className="mt-8 space-y-8">
              {Array.from({ length: 4 }).map((_, index) => (
                <div key={index}>
                  ...
                  <div className="mt-2 rounded-xl border border-[rgba(119,89,27,0.12)] bg-[#FFF9EA]/60 px-4 py-3">
                    {Array.from({ length: 4 }).map((__, rowIndex) => (
                      <div
                        key={rowIndex}
                        className="flex items-center justify-between gap-4 border-b border-[rgba(119,89,27,0.08)] py-2.5 last:border-b-0"
                      >
