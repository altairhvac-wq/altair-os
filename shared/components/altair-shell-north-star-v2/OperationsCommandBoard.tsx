...
import {
  ...
  missionWorkspaceSubheadingClass,
  missionLinkClass,
} from "./mission-tokens";
...
const techStateStyles = {
  on_job: { ring: "ring-cyan-200/80", bg: "from-cyan-50/80 to-white" },
  available: { ring: "ring-emerald-200/80", bg: "from-emerald-50/80 to-white" },
  break: { ring: "ring-slate-200/80", bg: "from-slate-50 to-white" },
  offline: { ring: "ring-slate-200/80", bg: "from-slate-100/60 to-white" },
} as const;
...
export function OperationsCommandBoard({
  ...
  return (
    <section aria-label="Operating command board" className={missionOperatingBoardClass}>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {connections.map((link) => (
              <div
                key={link.id}
                className="inline-flex items-center gap-1.5 rounded-lg bg-slate-800/50 px-2.5 py-1.5 ring-1 ring-slate-600/35"
              >
      ...
      <div className="grid lg:grid-cols-3 lg:divide-x lg:divide-slate-200/70">
        {/* ACTION NOW */}
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <p className={missionEyebrowLightClass}>Action now</p>
                </div>
              ...
              return (
                <li key={item.id}>
                  <Link href="/invoices" className={`group block ${missionRowClass}`}>
                    ...
                    <ArrowUpRight
                      className="h-3.5 w-3.5 shrink-0 text-slate-400 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-blue-600"
                      aria-hidden="true"
                    />
                  </Link>
                </li>
                ...
                return (
                  <li key={item.id}>
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate font-medium text-slate-700 group-hover:text-slate-900">
                        {item.title}
                      </span>
        ...
        {/* WORK MOVING */}
        <div className="relative flex flex-col gap-4 border-t border-slate-200/70 p-4 sm:p-5 lg:border-t-0 lg:p-6">
          <div className={missionColumnHeaderClass}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <Truck className="h-4 w-4 text-blue-600" aria-hidden="true" />
                  <p className={missionEyebrowLightClass}>Work moving</p>
                </div>
                <h3 className={`mt-1 ${missionWorkspaceSubheadingClass}`}>
                  {activeCount} calls active · {completedToday} ready to bill
                </h3>
                <p className="text-xs text-slate-600">Today&apos;s dispatch board · HVAC service & install</p>
              </div>
              <Link href="/dispatch" className={`shrink-0 ${missionLinkClass}`}>
                Dispatch
              </Link>
            </div>
          </div>
            ...
            <ul className="mt-2 grid grid-cols-2 gap-2">
              {technicians.map((tech) => {
                ...
                return (
                  ...
                  >
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-slate-100 text-[9px] font-semibold text-slate-800">
                      {tech.initials}
                    </span>
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-[10px] text-slate-600">{tech.jobLabel}</p>
                    </div>
                  </li>
                );
              })}
        ...
        <div className="relative flex flex-col gap-4 border-t border-slate-200/70 p-4 sm:p-5 lg:border-t-0 lg:p-6">
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <p className={missionEyebrowLightClass}>Money waiting</p>
                </div>
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-sm font-bold tabular-nums text-slate-900">{stage.amount}</span>
                    </div>
                    <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-slate-200/90">
                      <div
          ...
          <div className="mt-auto grid grid-cols-2 gap-2 border-t border-slate-200/70 pt-3">
            ...
            >
              ...
              <p className="text-[10px] text-slate-600">{expenseReview.pendingCount} to review</p>
            </Link>
            ...
            >
              ...
              <p className="mt-1 text-base font-bold tabular-nums text-slate-900">{leadOpportunity.value}</p>
              <p className="text-[10px] text-slate-600">{leadOpportunity.detail}</p>
            </Link>
          </div>
        </div>
      </div>
