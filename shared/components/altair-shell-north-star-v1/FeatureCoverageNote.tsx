...
export function FeatureCoverageNote({ entries }: FeatureCoverageNoteProps) {
  return (
    <details className="group rounded-xl bg-slate-100/80 ring-1 ring-slate-200/60">
      <summary className="cursor-pointer list-none px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 marker:content-none [&::-webkit-details-marker]:hidden">
        <span className="inline-flex items-center gap-2">
          <span className="rounded bg-slate-200 px-1.5 py-0.5 text-[9px] text-slate-600">Internal</span>
      ...
      <div className="border-t border-slate-200/60 px-4 py-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[32rem] text-left text-xs">
            ...
            <tbody>
              {entries.map((entry) => (
                <tr key={entry.feature} className="border-b border-slate-100 last:border-0">
                  <td className="py-2 pr-4 font-medium text-slate-700">{entry.feature}</td>
                  <td className="py-2 pr-4 text-slate-600">{entry.location}</td>
                  <td className="py-2 text-slate-500">{entry.productionSection ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
