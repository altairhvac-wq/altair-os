...
type ServiceItemQueueTabsProps = {
...
export function ServiceItemQueueTabs({
  activeQueue,
  onQueueChange,
  counts,
  northStar = false,
}: ServiceItemQueueTabsProps) {
  const tabs = SERVICE_ITEM_WORK_QUEUE_ORDER.map((queue) => ({
    id: queue,
    label: SERVICE_ITEM_WORK_QUEUE_LABELS[queue],
    count: counts[queue],
  }));
