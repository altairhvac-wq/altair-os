-- Remaining Phase 2 app schema (customers, jobs, billing, dispatch, network)
-- Prerequisites (must already exist before running this script):
--   - 001_core_auth.sql (companies, profiles, company_memberships, set_updated_at)
--   - RLS helpers from 002_app_core.sql: is_active_company_member(), has_company_role()
--   - Core auth RLS policies on companies, profiles, company_memberships
-- Does NOT include: bootstrap (003), membership self-read (004), customer search indexes (005)
-- Safe to re-run: uses IF NOT EXISTS / DROP IF EXISTS where PostgreSQL allows.
